...
public class EUExDevice extends EUExBase {
        ...
        try {
            ...
            if (line != null && line.length() > 0) {
                try {
                    ...
                    BDebug.log(
                            "EUExDeviceInfo---getCPUFrequency()---NumberFormatException ");
                }
            } else {
                result = "0";
            }
    ...
    private String getWIFISupport() {
        ...
        if ((WifiManager) mContext
                .getSystemService(Context.WIFI_SERVICE) != null) {// 支持WIFI
            supported = "1";
        }
        return supported;
    }
    ...
    /**
     * 获取本地mac地址
     * <uses-permission android:name="android.permission.ACCESS_WIFI_STATE"/>权限
     * 
     * @return
     */
    ...
    public void handleScreenCapture(String[] params) {
        if (params == null || params.length < 1) {
            Toast.makeText(mContext,
                    finder.getString("plugin_uexDevice_invalid_params"),
                    Toast.LENGTH_SHORT).show();
            return;
        }
        double quality = 1.0;
        ...
        try {
            ...
            String js = SCRIPT_HEADER + "if("
                    + CALLBACK_NAME_DEVICE_SCREEN_CAPTURE + "){"
                    + CALLBACK_NAME_DEVICE_SCREEN_CAPTURE + "('"
                    + jsonObject.toString() + "');}";
            onCallback(js);
    ...
    public void handleSetAudioCategory(String[] params) {
        ...
        AudioManager am = (AudioManager) ((Activity) mContext)
    ...
    public void handleGetVolume() {
        ...
        double result = bigDecimal.setScale(2, BigDecimal.ROUND_HALF_UP)
                .doubleValue();
        JSONObject jsonObject = new JSONObject();
        try {
        ...
        String js = SCRIPT_HEADER + "if(" + CALLBACK_NAME_DEVICE_GET_VOLUME
                + "){" + CALLBACK_NAME_DEVICE_GET_VOLUME + "('"
                + jsonObject.toString() + "');}";
        onCallback(js);
    }
    ...
    public void handleGetScreenBrightness() {
        ...
        String js = SCRIPT_HEADER + "if(" + CALLBACK_NAME_DEVICE_GET_BRIGHTNESS
                + "){" + CALLBACK_NAME_DEVICE_GET_BRIGHTNESS + "('"
                + jsonObject.toString() + "');}";
        onCallback(js);
    ...
    public void isFunctionEnable(final String[] params) {
        ...
        FunctionDataVO dataVO = DataHelper.gson.fromJson(params[0],
                FunctionDataVO.class);
        ...
        callBackPluginJs(JsConst.CALLBACK_IS_FUNCTION_ENABLE,
    ...
    public void openSetting(final String[] params) {
        ...
        callBackPluginJs(JsConst.CALLBACK_OPEN_SETTING,
                DataHelper.gson.toJson(resultVO));
    }
